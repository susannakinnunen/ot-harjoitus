...
def find_stretch(bodypart_name):
    ...
    cursor.execute("SELECT S.name, S.description FROM Stretches S WHERE id=:stretch_id", {"stretch_id":stretch_id})
...
def get_stretch_id_by_name(stretch_name):
    ...
    result = cursor.execute("SELECT id FROM Stretches WHERE name=:name", {"name":stretch_name})
    stretch_id_result = result.fetchone()
    strech_id = stretch_id_result[0]
    return strech_id
...
class Stretch:
    ...
    def add_stretch(self, name, description):
        ...
        result = cursor.execute(sql, {"name":name, "description":description})
