...
class StretchingService:
    ...
    def add_bodypart(self, name, stretch):
        """ Kutsuu BodypartRepository-luokkaa tallentamaan
        kehonsa- ja venytystietoja csv.tiedostoon ja tietokantaan. """
        return self.bodypart_repository.write_bodyparts_to_file_and_database(
            name, stretch)

    def add_stretch(self, bodypart_name, description):
        """ Kutsuu StretchRepository-luokkaa tallentamaan
        kehonsa- ja venytystietoja csv.tiedostoon ja tietokantaan. """
    ...
    def add_combination(self, name, stretch):
        """ Kutsuu BodypartStretch-luokkaa lisäämään
        yhdistämään tietokannassa tietyn kehonosan ja venytyksen id:t"""
        return self.bodypart_stretch_repository.add_combination(name, stretch)

    def get_all_bodyparts(self):
        """ Kutsuu BodypartRepositiry-luokkaa
         hakemaan kaikki kehonosat"""
        return self.bodypart_repository.find_all()

    def show_stretch(self, bodypart):
        """ Kutsuu StretchRepository-luokkaa hakemaan
         venytyksen tietyn kehonosan perusteella"""
        return self.stretch_repository.find_by_bodypart(bodypart)

    def combine_stretches_and_bodyparts(self):
        """ Kutsuu BodypartRepository-luokkaa yhdistämään
         venytyksen ja kehonosan csv.tiedostosta tietokantaan.
         Tehdään ohjelman käynnistyksen yhteydessä. """
        self.bodypart_stretch_repository.get_bodyparts_and_stretches_from_file()

    def create_new_user(self, username, password):
        """ Kutsuu UserRpository-luokkaa luomaan uuden
         käyttäjätunnuksen. Tarkistaa onko käyttäjä ylläpitäjä.
         Palauttaa tiedon siitä, onko käyttäjätunnus jo olemassa, jolloin
    ...
    def login(self, username, password):
        """ Kutsuu UserRepository-luokkaa kirjautumaan
         sisään tietyllä käyttäjätunnuksella ja salasanalla. """
        login = self.user_repository.login(username, password)
        return login

    def check_if_admin(self, username):
        """Tarkistaa onko käyttäjä ylläpitäjä.
